...
for rewards, learning rates `b` for punishments, action probability
...
"""
LRPLearner(n::Int, W::Vector{Float64}, a::Float64, b::Float64) = LRPLearner(n, W, repeat([a], outer=n), repeat([b], outer=n))


"""
    LRPLearner(n::Int, W::Vector{Float64}, a::Vector{Float64}, b::Vector{Float64})

Create an `LRPLearner` with `n` actions, initial action probability vector `W`,
and learning rate vectors `a` and `b`.
"""
function LRPLearner(n::Int, W::Vector{Float64}, a::Vector{Float64}, b::Vector{Float64})
    R = Vector{Matrix{Float64}}(undef, 0)
    P = Vector{Matrix{Float64}}(undef, 0)
    
    # construct reward and penalty matrices
    for i in 1:n
        push!(R, (1 - a[i]) * LinearAlgebra.I + a[i] * matrixunit(n, i) * ones(n, n))
        push!(P, (1 - b[i]) * LinearAlgebra.I + (b[i]/(n - 1)) * (ones(n, n) - matrixunit(n, i)* ones(n, n)))
    end
